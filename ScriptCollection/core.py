...
class ScriptCollection:
    ...
    def SCCreateRelease(self, configurationfile: str) -> int:
        ...
        prepare = self.get_buildscript_config_boolean_value(configparser, 'general', 'prepare')
        repository_version = self.get_version_for_buildscripts(configparser)
        repository = self.get_buildscript_config_item(configparser, "general", "repository")
        write_message_to_stdout(f"Create release v{repository_version} for repository {repository}")
        releaserepository = self.get_buildscript_config_item(configparser, "other", "releaserepository")

        if (self._private_repository_has_changes(repository) or self._private_repository_has_changes(releaserepository)):
            return 1

        if prepare:
            devbranch = self.get_buildscript_config_item(configparser, 'prepare', 'developmentbranchname')
            masterbranch = self.get_buildscript_config_item(configparser, 'prepare', 'masterbranchname')
        ...
        if error_occurred:
            if prepare:
                ...
                self.git_checkout(repository, self.get_buildscript_config_item(configparser, 'prepare', 'developmentbranchname'))
            write_message_to_stderr("Building wheel and running testcases was not successful")
            return 1
        else:
            if prepare:
                commit_id = self.git_commit(repository, "Merge branch '" + self.get_buildscript_config_item(configparser, 'prepare', 'developmentbranchname')+"' into '" + self.get_buildscript_config_item(configparser, 'prepare', 'masterbranchname')+"'")
                self.git_create_tag(repository, commit_id, self.get_buildscript_config_item(configparser, 'prepare', 'gittagprefix') + repository_version)
                self.git_merge(repository, self.get_buildscript_config_item(configparser, 'prepare', 'masterbranchname'), self.get_buildscript_config_item(configparser, 'prepare', 'developmentbranchname'), True)
                if self.get_buildscript_config_boolean_value(configparser, 'other', 'exportrepository'):
                    branch = self.get_buildscript_config_item(configparser, 'prepare', 'masterbranchname')
                    self.git_push(repository, self.get_buildscript_config_item(configparser, 'other', 'exportrepositoryremotename'), branch, branch, False, True)
                self.git_commit(self.get_buildscript_config_item(configparser, 'other', 'releaserepository'), "Added "+self.get_buildscript_config_item(configparser, 'general', 'productname')+" "+self.get_buildscript_config_item(configparser, 'prepare', 'gittagprefix')+repository_version)
    ...
    def SCDotNetCreateExecutableRelease(self, configurationfile: str) -> int:
        ...
        configparser.read_file(open(configurationfile, mode="r", encoding="utf-8"))
        repository_version = self.get_version_for_buildscripts(configparser)
        if self.get_buildscript_config_boolean_value(configparser, 'dotnet', 'updateversionsincsprojfile'):
            update_version_in_csproj_file(self.get_buildscript_config_item(configparser, 'dotnet', 'csprojfile'), repository_version)
    ...
    def SCDotNetBuildNugetAndRunTests(self, configurationfile: str) -> int:
        ...
        copy_tree(self.get_buildscript_config_item(configparser, 'dotnet', 'buildoutputdirectory'), publishdirectory_binary)
        nuspec_content = self._private_replace_underscores_for_buildconfiguration(self._private_nuget_template, configparser)
        nuspecfilename = self.get_buildscript_config_item(configparser, 'general', 'productname')+".nuspec"
    ...
    def SCDotNetReleaseNuget(self, configurationfile: str) -> int:
        ...
        repository_version = self.get_version_for_buildscripts(configparser)
        publishdirectory = self.get_buildscript_config_item(configparser, 'dotnet', 'publishdirectory')
        latest_nupkg_file = self.get_buildscript_config_item(configparser, 'general', 'productname')+"."+repository_version+".nupkg"
        for localnugettarget in self.get_buildscript_config_items(configparser, 'dotnet', 'localnugettargets'):
        ...
        for localnugettargetrepository in self.get_buildscript_config_items(configparser, 'dotnet', 'localnugettargetrepositories'):
            self.git_commit(localnugettargetrepository, f"Added {self.get_buildscript_config_item(configparser,'general','productname')} .NET-release {self.get_buildscript_config_item(configparser,'prepare','gittagprefix')}{repository_version}")
        if (self.get_buildscript_config_boolean_value(configparser, 'dotnet', 'publishnugetfile')):
            with open(self.get_buildscript_config_item(configparser, 'dotnet', 'nugetapikeyfile'), 'r', encoding='utf-8') as apikeyfile:
                api_key = apikeyfile.read()
            nugetsource = self.get_buildscript_config_item(configparser, 'dotnet', 'nugetsource')
    ...
    def SCDotNetReference(self, configurationfile: str) -> int:
        ...
        if self.get_buildscript_config_boolean_value(configparser, 'dotnet', 'generatereference'):
            if self.get_buildscript_config_boolean_value(configparser, 'other', 'verbose'):
                ...
                verbose_argument = 1
            docfx_file = self.get_buildscript_config_item(configparser, 'dotnet', 'docfxfile')
            ...
            coveragefolder = self.get_buildscript_config_item(configparser, 'dotnet', 'coveragefolder')
            ...
            self.git_commit(self.get_buildscript_config_item(configparser, 'dotnet', 'referencerepository'), "Updated reference")
            if self.get_buildscript_config_boolean_value(configparser, 'dotnet', 'exportreference'):
    ...
    def SCDotNetRunTests(self, configurationfile: str) -> int:
        ...
        configparser = ConfigParser()
        configparser.read_file(open(configurationfile, mode="r", encoding="utf-8"))
        runtime = self.get_buildscript_config_item(configparser, 'dotnet', 'testruntime')
        if self.get_buildscript_config_boolean_value(configparser, 'other', 'verbose'):
            verbose_argument_for_dotnet = "detailed"
            verbose_argument = 2
        else:
            verbose_argument_for_dotnet = "normal"
            verbose_argument = 1
        self.SCDotNetBuild(self._private_get_test_csprojfile_folder(configparser), self._private_get_test_csprojfile_filename(configparser), self.get_buildscript_config_item(configparser, 'dotnet', 'testoutputfolder'), self.get_buildscript_config_item(configparser, 'dotnet', 'buildconfiguration'), runtime, self.get_buildscript_config_item(configparser, 'dotnet', 'testdotnetframework'), True, verbose_argument, None, None)
        self.execute_and_raise_exception_if_exit_code_is_not_zero("dotnet", "test "+self._private_get_test_csprojfile_filename(configparser)+" -c " + self.get_buildscript_config_item(configparser, 'dotnet', 'buildconfiguration') + f" --verbosity {verbose_argument_for_dotnet} /p:CollectCoverage=true /p:CoverletOutput=" + self._private_get_coverage_filename(configparser)+" /p:CoverletOutputFormat=opencover", self._private_get_test_csprojfile_folder(configparser), 3600, verbose_argument, False, "Execute tests")
        return 0
    ...
    def SCPythonCreateWheelRelease(self, configurationfile: str) -> int:
        ...
        configparser.read_file(open(configurationfile, mode="r", encoding="utf-8"))
        repository_version = self.get_version_for_buildscripts(configparser)
        if(self.get_buildscript_config_boolean_value(configparser, 'python', 'updateversion')):
    ...
    def SCPythonBuild(self, configurationfile: str) -> int:
        ...
        setuppyfile = self.get_buildscript_config_item(configparser, "python", "pythonsetuppyfile")
        setuppyfilename = os.path.basename(setuppyfile)
        setuppyfilefolder = os.path.dirname(setuppyfile)
        publishdirectoryforwhlfile = self.get_buildscript_config_item(configparser, "python", "publishdirectoryforwhlfile")
    ...
    def SCPythonRunTests(self, configurationfile: str) -> int:
        ...
        configparser.read_file(open(configurationfile, mode="r", encoding="utf-8"))
        if self.get_buildscript_config_boolean_value(configparser, 'other', 'hastestproject'):
            pythontestfile = self.get_buildscript_config_item(configparser, 'python', 'pythontestfile')
    ...
    def SCPythonReleaseWheel(self, configurationfile: str) -> int:
        ...
        if self.get_buildscript_config_boolean_value(configparser, 'python', 'publishwhlfile'):
            with open(self.get_buildscript_config_item(configparser, 'python', 'pypiapikeyfile'), 'r', encoding='utf-8') as apikeyfile:
                api_key = apikeyfile.read()
            gpgidentity = self.get_buildscript_config_item(configparser, 'other', 'gpgidentity')
            repository_version = self.get_version_for_buildscripts(configparser)
            productname = self.get_buildscript_config_item(configparser, 'general', 'productname')
            if self.get_buildscript_config_boolean_value(configparser, 'other', 'verbose'):
            ...
            self.execute_and_raise_exception_if_exit_code_is_not_zero("twine", twine_argument, self.get_buildscript_config_item(configparser, "python", "publishdirectoryforwhlfile"), 3600, self._private_get_verbosity_for_exuecutor(configparser))
        return 0
    ...
    def get_commit_ids_between_dates(self, directory: str, since: datetime, until: datetime):
    ...
    def git_fetch(self, folder: str, remotename: str = "--all", printErrorsAsInformation: bool = True):
    ...
    def git_push(self, folder: str, remotename: str, localbranchname: str, remotebranchname: str, forcepush: bool = False, pushalltags: bool = False):
    ...
    def git_clone_if_not_already_done(self, clone_target_folder: str, remote_repository_path: str, include_submodules: bool = True, mirror: bool = False):
    ...
    def git_add_or_set_remote_address(self, directory: str, remote_name: str, remote_address: str):
    ...
    def git_discard_all_unstaged_changes(self, directory: str):
        """Caution: This function executes 'git clean -df'. This can delete files which maybe should not be deleted. Be aware of that."""
        self.execute_and_raise_exception_if_exit_code_is_not_zero("git", f'clean -df', directory, 3600, 1, False, "Discard", False)
        self.execute_and_raise_exception_if_exit_code_is_not_zero("git", f'checkout -- .', directory, 3600, 1, False, "Discard", False)

    def git_commit(self, directory: str, message: str, author_name: str = None, author_email: str = None, stage_all_changes: bool = True, allow_empty_commits: bool = False):
        do_commit = False
    ...
    def git_create_tag(self, directory: str, target_for_tag: str, tag: str):
    ...
    def _private_verbose_check_for_not_available_item(self, configparser: ConfigParser, queried_items: list, section: str, propertyname: str):
        if self.get_buildscript_config_boolean_value(configparser, 'other', 'verbose'):
    ...
    def _private_get_buildoutputdirectory(self, configparser: ConfigParser, runtime: str) -> str:
        result = self.get_buildscript_config_item(configparser, 'dotnet', 'buildoutputdirectory')
        if self.get_buildscript_config_boolean_value(configparser, 'dotnet', 'separatefolderforeachruntime'):
            result = result+os.path.sep+runtime
        return result

    def get_buildscript_config_boolean_value(self, configparser: ConfigParser, section: str, propertyname: str) -> bool:
        try:
    ...
    def get_buildscript_config_items(self, configparser: ConfigParser, section: str, propertyname: str, custom_replacements: dict = {}, include_version=True) -> list:
    ...
    def _private_get_csprojfile_filename(self, configparser: ConfigParser) -> str:
        file = self.get_buildscript_config_item(configparser, "dotnet", "csprojfile")
    ...
    def _private_get_test_csprojfile_filename(self, configparser: ConfigParser) -> str:
        file = self.get_buildscript_config_item(configparser, "dotnet", "testcsprojfile")
        file = resolve_relative_path_from_current_working_directory(file)
        result = os.path.basename(file)
        return result
    ...
    def _private_get_coverage_filename(self, configparser: ConfigParser) -> str:
        return self.get_buildscript_config_item(configparser, "general", "productname")+".TestCoverage.opencover.xml"

    def get_version_for_buildscripts(self, configparser: ConfigParser) -> str:
        return self.get_version_for_buildscripts_helper(self.get_buildscript_config_item(configparser, 'general', 'repository', {}, False))

    @lru_cache(maxsize=None)
    def get_version_for_buildscripts_helper(self, folder: str) -> str:
        return self.get_semver_version_from_gitversion(folder)

    def _private_replace_underscore_in_file_for_buildconfiguration(self, file: str, configparser: ConfigParser, replacements: dict = {}, encoding="utf-8"):
    ...
    def _private_create_thumbnails(self, filename: str, length_in_seconds: float, amount_of_images: int, folder: str, tempname_for_thumbnails):
    ...
    def _private_create_thumbnail(self, outputfilename: str, folder: str, length_in_seconds: float, tempname_for_thumbnails):
    ...
    def SCCreateHashOfAllFiles(self, folder: str):
    ...
    def _private_merge_files(self, sourcefile: str, targetfile: str):
    ...
    def _private_process_file(self, file: str, substringInFilename: str, newSubstringInFilename: str, conflictResolveMode: str):
    ...
    def SCReplaceSubstringsInFilenames(self, folder: str, substringInFilename: str, newSubstringInFilename: str, conflictResolveMode: str):
        for file in absolute_file_paths(folder):
            self._private_process_file(file, substringInFilename, newSubstringInFilename, conflictResolveMode)

    def _private_check_file(self, file: str, searchstring: str):
    ...
    def SCSearchInFiles(self, folder: str, searchstring: str):
        for file in absolute_file_paths(folder):
            self._private_check_file(file, searchstring)

    def _private_print_qr_code_by_csv_line(self, line: str):
        splitted = line.split(";")
    ...
    def SCShow2FAAsQRCode(self, csvfile: str):
        separator_line = "--------------------------------------------------------"
    ...
    def SCCalculateBitcoinBlockHash(self, version: str, previousblockhash: str, transactionsmerkleroot: str, timestamp: str, target: str, nonce: str) -> str:
        # Example-values:
    ...
    def SCChangeHashOfProgram(self, inputfile: str):
    ...
    def _private_create_iso(self, folder, iso_file):
        created_directories = []
    ...
    def SCCreateISOFileWithObfuscatedFiles(self, inputfolder: str, outputfile: str, printtableheadline, createisofile, extensions):
    ...
    def SCFilenameObfuscator(self, inputfolder: str, printtableheadline, namemappingfile: str, extensions: str):
    ...
    def SCObfuscateFilesFolder(self, inputfolder: str, printtableheadline, namemappingfile: str, extensions: str):
        obfuscate_all_files = extensions == "*"
        if(not obfuscate_all_files):
            if "," in extensions:
                obfuscate_file_extensions = extensions.split(",")
    ...
    def update_nuget_package(self, csproj_file: str, name: str):
    ...
    def _private_log_program_start(self, program: str, arguments: str, workingdirectory: str, verbosity: int = 1):
        if(verbosity == 2):
...
def update_version_in_csproj_file(file: str, version: str) -> None:
