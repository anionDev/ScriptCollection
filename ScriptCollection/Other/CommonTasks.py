...
def common_tasks():
    ...
    targetenvironmenttype = tfcps.get_targetenvironmenttype_from_commandline_arguments(cmd_args, "QualityCheck")
    additional_arguments_file = tfcps.get_additionalargumentsfile_from_commandline_arguments(cmd_args, None)
    version = tfcps.get_version_of_project(GeneralUtilities.resolve_relative_path("../..", folder_of_current_file))
    ...
    tfcps.standardized_tasks_do_common_tasks(file, version, verbosity, targetenvironmenttype, True, additional_arguments_file, sys.argv)
