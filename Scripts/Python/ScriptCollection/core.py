...
def get_direct_files_of_folder(folder: str):
    return [os.path.join(folder, f) for f in listdir(folder) if isfile(join(folder, f))]


def get_direct_folders_of_folder(folder: str):
    return [os.path.join(folder, f) for f in listdir(folder) if isdir(join(folder, f))]
...
def clone_folder_structure(source: str, target: str, write_information_to_file):
    ...
    for source_file in absolute_file_paths(source):
        ...
        with open(target_file, 'w', encoding='utf8') as file:
            file.write(get_metadata_for_file_for_clone_folder_structure(source_file))
